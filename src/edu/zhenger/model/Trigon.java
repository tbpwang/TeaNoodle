...
public class Trigon implements Cell
{
    ...
    public Trigon(LatLon top, LatLon left, LatLon right, String geocode)
    {
        ...
        latLons[2] = right;
        Vec4 a, b, c, d;
        a = Term.fromLatLon(top);
        b = Term.fromLatLon(left);
        c = Term.fromLatLon(right);
        ...
        this.center = new LatLon(Term.fromVec4(d));
    ...
    {
        return getArea(Term.getSphere());
    }

    public double getPerimeter()
    {
        return getPerimeter(Term.getSphere());
    }

    @Override
    public double computeCompactness()
    {
        return getArea(Term.getSphere()) / getPerimeter(Term.getSphere());
    }
...
//        return "Cell{" +
//            "geocode=" + geocode +
//            ", topV=" + Term.getInstance().fromVec4(topV).toString() +
//            ", leftV=" + Term.getInstance().fromVec4(leftV).toString() +
//            ", rightV=" + Term.getInstance().fromVec4(rightV).toString() +
//            '}';
        return geocode.substring(1) + "\t" + this.getArea(Term.getSphere()) + "\t"
...
//    }
    ...
    {
        return Term.fromVec4(topV);
    }

    //@Override
    public LatLon getLeft()
    {
        return Term.fromVec4(leftV);
    }

    //@Override
    public LatLon getRight()
    {
        return Term.fromVec4(rightV);
    }

    @Override
    public Globe getGlobe()
    {
        return Term.getSphere();
    }
    ...
    public double getArea(Globe globe)
    {
        ...
        LatLon top, left, right, lTop, rTop;
        top = Term.fromVec4(topV);
        left = Term.fromVec4(leftV);
        right = Term.fromVec4(rightV);
        if (Math.abs(top.latitude.getDegrees()) == 90.0)
        {
...
//        double c = topV.distanceTo2(leftV);
    ...
    public double getHeight(Globe globe)
    {
        ...
        LatLon latLonA = Term.fromVec4(topV);
        LatLon latLonB = Term.fromVec4(leftV);
        LatLon latLonC = Term.fromVec4(rightV);
...
////        LatLon latLonA = Term.getInstance().fromVec4(topV);
////        LatLon latLonB = Term.getInstance().fromVec4(leftV);
////        LatLon latLonC = Term.getInstance().fromVec4(rightV);
//        double a = Math.abs(leftV.angleBetween3(rightV).getRadians());
//        double b = Math.abs(rightV.angleBetween3(topV).getRadians());
//        double c = Math.abs(topV.angleBetween3(leftV).getRadians());
