    ...
    public static class AppFrame extends ApplicationTemplate.AppFrame
    {
        public AppFrame()
        {
            ...
            this.getWwd().getView().setEyePosition(
                new Position(LatLon.getCenter(first2levels().get(0).getLocations(Term.getSphere())),
...
//            levels.add(children[3].surfacePolygon());
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    levels.add(
                        ((Trigon) trigon.subdivide().getChildren()[i].subdivide().getChildren()[j]).getSurfacePolygon());
                }
            }
