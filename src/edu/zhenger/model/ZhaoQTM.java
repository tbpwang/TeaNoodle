...
//        LatLon[][] latLons = new LatLon[lines + 1][];
        ...
        double dCol = 1.0;
//        double delt;
//        if (trigon.getRight().longitude.multiply(trigon.getLeft().longitude.degrees).degrees < 0)
//        {
//            delt = trigon.getRight().longitude.degrees + trigon.getLeft().longitude.degrees;
//        }
//        else
//        {
//            delt = trigon.getRight().longitude.subtract(trigon.getLeft().longitude).degrees;
//        }
        ...
        for (int i = 0; i < lines + 1; i++)
        {
            ...
            {
//                lats[0] = 90.0;
//                lats[0] = trigon.getTop().latitude.getRadians();
                lats[0] = trigon.getLatLons()[0].latitude.radians;
//                System.out.println(trigon.getTop().latitude.degrees);
            }
            else if (i == lines)
            {
//                lats[i] = 0.0;
//                lats[i] = trigon.getLeft().latitude.getRadians();
            ...
            for (int j = 0; j < i + 1; j++)
            {
                ...
                {
                    latLons[i][0] = LatLon.fromDegrees(Term.radian2Degree(lats[i]),
                        trigon.getLatLons()[1].longitude.degrees);
                }
                else if (j == i)
                {
                    latLons[i][j] = LatLon.fromDegrees(Term.radian2Degree(lats[i]),
...
//        return latLons;
    ...
    private LatLon[][] initView(Trigon trigon)
    {
        ...
        if (trigon.getTop().latitude.degrees == 90.0)
        {
            double[] lats = new double[lines + 1];
            double dCol = 1.0;
            double delt = trigon.getRight().longitude.subtract(trigon.getLeft().longitude).degrees;
            for (int i = 0; i < lines + 1; i++)
            {
                if (i == 0)
                {
                ...
                for (int j = 0; j < i + 1; j++)
                {
                    if (i == 0)
                    {
                        temp[0][0] = new LatLon(trigon.getTop());
                    }
                    else if (j == 0)
                    {
                        temp[i][0] = LatLon.fromRadians(lats[i], trigon.getLeft().longitude.radians);
                    }
                    else if (j == i)
                    {
                        temp[i][j] = LatLon.fromRadians(lats[i], trigon.getRight().longitude.radians);
                    }
                    else
                    {
                        temp[i][j] = LatLon.fromRadians(lats[i],
                            trigon.getLeft().longitude.addDegrees(j * dCol).degrees);
                    }
                }
            }
        }
        ...
        {
            LatLon A, B, C;
            A = new LatLon(trigon.getTop().latitude.multiply(-1.0), trigon.getTop().longitude);
            B = new LatLon(trigon.getLeft().latitude.multiply(-1.0), trigon.getLeft().longitude);
            C = new LatLon(trigon.getRight().latitude.multiply(-1.0), trigon.getRight().longitude);
