...
public class QTM implements Grid
{
    ...
    {
        if (computeRanks(cell).getLevel() != this.level)
        {
            String message = Logging.getMessage("Overflow.CellIsOutOfGrid");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
...
//            throw new Mistake("CellIsNull");
        ...
        int i = computeRanks(cell).getRow();
        int j = computeRanks(cell).getCol();
    ...
    public Cell search(int row, int column)
    {
        if (ranksCells == null)
        {
            String message = Logging.getMessage("nullValue.SetIsNull");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
...
//        double distance = Double.MAX_VALUE;
        ...
        if (!isNeighbor(cell1, cell2))
        {
            String message = Logging.getMessage("parameterError.NeighborDoesNotExist");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
        ...
        {
            int r1 = computeRanks(cell1).getRow();
            int r2 = computeRanks(cell2).getRow();
            if (r1 == r2)
            {
            ...
            vec41 = Term.fromLatLon(p1);
            vec42 = Term.fromLatLon(p2);
    ...
    private boolean isNeighbor(Cell cell1, Cell cell2)
    {
        ...
        Neighbor n = (Neighbor) adjoin(cell1);
