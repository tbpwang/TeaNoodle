    ...
    private static List<Vec4> subdivision(Triangle triangle, int level)
    {
        ...
        for (int i = 0; i < level; i++)
        {
                ...
                triangleList)
            {
                ...
                vB = t.getB();
                vC = t.getC();
                lA = Cons.vec4ToLatLon(vA);
                lB = Cons.vec4ToLatLon(vB);
                lC = Cons.vec4ToLatLon(vC);
                ...
                vASub = Cons.check(Cons.latLonToVec4(lASub));
                vBSub = Cons.check(Cons.latLonToVec4(lBSub));
                vCSub = Cons.check(Cons.latLonToVec4(lCSub));
                vec4List.add(vASub);
                vec4List.add(vBSub);
                vec4List.add(vCSub);
